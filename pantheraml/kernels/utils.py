...
if DEVICE_TYPE == "xpu" and Version(torch.__version__) < Version("2.6.0"):
    raise RuntimeError("Intel xpu currently supports PantheraML with torch.version >= 2.6.0")
...
def get_gpu_memory_info(device_id=None):
    ...
    if device_id is None:
        device_id = get_current_gpu_index()
    ...
    if DEVICE_TYPE == "cuda":
        ...
        return {
            ...
            'max_allocated': torch.xpu.max_memory_allocated(device_id),
        }
    return {'allocated': 0, 'reserved': 0, 'max_allocated': 0}

def clear_gpu_cache():
    """Clear GPU cache for current device."""
    if DEVICE_TYPE == "cuda":
...
def get_tpu_memory_info():
    """
    Get TPU memory information (EXPERIMENTAL).
    
    Returns:
        dict: TPU memory statistics or empty dict if TPU not available.
    """
    try:
        import torch_xla.core.xla_model as xm  # type: ignore
        
        # TPU memory tracking is limited compared to CUDA
        return {
        ...
        return {'available': False, 'error': 'torch_xla not installed'}
...
def optimize_tpu_memory():
    """
    Apply TPU-specific memory optimizations (EXPERIMENTAL).
    """
    try:
        import torch_xla.core.xla_model as xm  # type: ignore
        import os
        
        print("🧪 EXPERIMENTAL: Applying TPU memory optimizations...")
        
        # Set TPU-specific environment variables
        os.environ["XLA_USE_BF16"] = "1"
        os.environ["XLA_TENSOR_ALLOCATOR_MAXSIZE"] = "100000000"
        
        # Force synchronization to ensure settings take effect
        xm.mark_step()
        
        print("   ✅ TPU memory optimizations applied")
        
    except ImportError:
        print("   ⚠️  TPU optimizations skipped: torch_xla not available")
    except Exception as e:
        print(f"   ⚠️  TPU optimization warning: {e}")


def get_device_memory_info(device_type: str = None):
    """
    Get memory information for any device type.
    
    Args:
        device_type: 'cuda', 'xpu', 'tpu', or None for auto-detect
        
    Returns:
        dict: Memory information for the specified device type.
    """
