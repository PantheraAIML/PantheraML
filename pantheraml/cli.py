...
"""
🦥 Starter Script for Fine-Tuning FastLanguageModel with PantheraML

PantheraML is based on and extends the excellent work of the Unsloth team.
We gratefully acknowledge their contributions to efficient LLM fine-tuning.

This script is designed as a starting point for fine-tuning your models using PantheraML.
...
Usage: (most of the options have valid default values this is an extended example for demonstration purposes)
    python pantheraml-cli.py --model_name "unsloth/llama-3-8b" --max_seq_length 8192 --dtype None --load_in_4bit \
...
def run(args):
    try:
        ...
        if "PantheraML currently only works on NVIDIA GPUs" in str(e):
            ...
            print("💡 Current system is not supported for training")
            print(f"\nError details: {e}")
            exit(1)
        else:
            raise
...
def run_benchmark(args):
    ...
    try:
        ...
        if "PantheraML currently only works on NVIDIA GPUs" in str(e):
            print("❌ Device Compatibility Error")
            print("🚫 PantheraML requires NVIDIA GPUs, Intel GPUs, or TPUs")
            print("💡 Current system is not supported for benchmarking")
            print(f"\nError details: {e}")
            exit(1)
        else:
            raise
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="🦥 Fine-tune your LLM faster using PantheraML! (Built on Unsloth's foundation)",
...
Happy fine-tuning! 🎯
    ...
    print("🦥" + "="*70)
    print("🚀 PantheraML CLI - Enhanced LLM Fine-tuning")
    print("   Based on the excellent work of the Unsloth team")
    print("   With added multi-GPU support and enhanced features")
    print("="*72)
    print()
...
if __name__ == "__main__":
    try:
        ...
        if "PantheraML currently only works on NVIDIA GPUs" in str(e):
            ...
            print("🚫 PantheraML requires NVIDIA GPUs, Intel GPUs, or TPUs")
            print("💡 Current system is not supported for training")
            print("📝 You can still view help and configure options with --help")
            print(f"\nError details: {e}")
            exit(1)
        else:
            raise
