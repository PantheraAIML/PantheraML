        ...
        if len(past_key_values) == 0:
            ...
            if hasattr(base_model, "_prepare_4d_causal_attention_mask_with_cache_position"):
                ...
                try:
                    ...
                    print(f"PantheraML: Could not inspect signature of {base_model._prepare_4d_causal_attention_mask_with_cache_position}")
...
def LlamaModel_fast_forward(
    ...
    *args, **kwargs,
    ...
    if input_ids is not None and inputs_embeds is not None:
        raise ValueError("PantheraML: You cannot specify both decoder_input_ids and decoder_inputs_embeds at the same time")
    elif input_ids is not None:
        batch_size, seq_length = input_ids.shape
    elif inputs_embeds is not None:
        batch_size, seq_length, _ = inputs_embeds.shape
    else:
        raise ValueError("PantheraML: You have to specify either decoder_input_ids or decoder_inputs_embeds")
    ...
    if hasattr(self, "max_seq_length"):
        if seq_length > self.max_seq_length:
            logger.warning_once(
                f"PantheraML: Input IDs of length {seq_length} > the model's max sequence length of {self.max_seq_length}.\n"\
...
class FastLlamaModel:
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        statistics = \
        f"==((====))==  PantheraML {__version__}: Fast {model_patcher.__name__[4:-5]} patching. Transformers: {transformers_version}.{vllm_version}\n"\
        ...
        f"==((====))==  PantheraML - 2x faster free finetuning | Num GPUs used = {len(set(p.device for p in model.parameters()))}\\n"\\
        ...
        if n_total_devices > 1:
            logger.warning_once('PantheraML is running with multi GPUs - the effective batch size is multiplied by ' + str(n_total_devices))
        debug_info ="""
        debug_info = debug_info.split('\n')
        debug_info = "\n".join([debug_info[0]] + [spaces + x[8:] for x in debug_info[1:]])
    ...
    def patch_peft_model(
        ...
        logger.warning_once(
            f"PantheraML {__version__} patched {len(model.model.model.layers)} layers with "\
