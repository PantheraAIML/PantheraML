...
class FastLanguageModel(FastLlamaModel):
    ...
    def from_pretrained(
        ...
        max_memory                 = None,
        use_multi_gpu              = None,  # Auto-detect if None
        
        # Experimental TPU parameters  
        use_tpu                    = None,  # Auto-detect if None
        ...
        *args, **kwargs,
        ...
        try:
            ...
            if use_tpu is None:
                use_tpu = True
                print("🧪 EXPERIMENTAL: TPU detected, enabling TPU support")
                print("   ⚠️  Note: TPU support is experimental and may have limitations")
        except ImportError:
            if use_tpu:
                raise RuntimeError(
                    "🧪 EXPERIMENTAL: TPU support requested but torch_xla not available. "
                    "Install with: pip install torch_xla"
                )
            use_tpu = False
        
        # Handle device mapping for TPU
        if use_tpu and tpu_available:
            device_map = "auto"  # Let XLA handle device placement
            print(f"🧪 EXPERIMENTAL: Using TPU with {tpu_cores} cores")
        
        # Auto-detect multi-GPU usage
        if use_multi_gpu is None:
            use_multi_gpu = num_gpus > 1 and is_distributed_available()
        
        # Optimize device mapping for multi-GPU
        if use_multi_gpu and auto_device_map and num_gpus > 1:
            if device_map == "sequential":
                # Keep sequential for multi-GPU
                pass
            ...
            if is_main_process():
                print(f"🔧 Multi-GPU setup detected: Using {num_gpus} GPUs")
                print(f"   Device mapping strategy: {device_map}")
                if max_memory:
        ...
        if fast_inference:
            if importlib.util.find_spec("vllm") is None:
                raise ImportError(
                    "PantheraML: Please install vLLM before enabling `fast_inference`!\n"\
    ...
    def from_pretrained(
        ...
        *args, **kwargs,
        ...
        if full_finetuning and (load_in_4bit or load_in_8bit):
            print("PantheraML: You selected full finetuning support, but 4bit / 8bit is enabled - disabling LoRA / QLoRA.")
            load_in_4bit = False
            load_in_8bit = False
        pass
        ...
        if load_in_4bit and load_in_8bit:
            raise RuntimeError(
                "PantheraML: Can only load in 4bit or 8bit, not both!\n"\
        ...
        if "pixtral" in lowered_model_name and transformers_version < Version("4.49.0"):
            raise RuntimeError("PantheraML: Pixtral only works on transformers >= 4.49.0." + LATEST)
        # Qwen 2.5
        elif "qwen2.5" in lowered_model_name and transformers_version < Version("4.49.0"):
            raise RuntimeError("PantheraML: Qwen 2.5 only works on transformers >= 4.49.0." + LATEST)
        # Gemma 3
        elif "gemma-3" in lowered_model_name and transformers_version < Version("4.50.0.dev0"):
            raise RuntimeError("PantheraML: Gemma 3 only works on transformers >= 4.50.0." + NIGHTLY)
        # Cohere
        elif "c4ai-command-a-03-2025" in lowered_model_name and transformers_version < Version("4.50.0.dev0"):
            raise RuntimeError("PantheraML: Cohere's Command model only works on transformers >= 4.50.0." + NIGHTLY)
        # Sesame
        ...
        # Olmo 2
            ...
            raise RuntimeError("PantheraML: OLMo-2 only works on transformers >= 4.50.0." + NIGHTLY)
        # Gemma 3N
            ...
            for check_model_name in DISABLE_COMPILE_MODEL_NAMES:
                if check_model_name in lowered_model_name:
                    ...
                    if transformers_version < Version("4.50.0.dev0"):
                        raise RuntimeError(f"PantheraML: {check_model_name} only works on transformers >= 4.50.0." + NIGHTLY)
                    break
        pass
