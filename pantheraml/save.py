...
def check_if_sentencepiece_model(model, temporary_location = "_pantheraml_sentencepiece_temp"):
...
def _free_cached_model(model):
    ...
    for cached_repo in cached_repos:
        if cached_repo.repo_id == model.config._name_or_path:
            ...
            logger.warning_once(
                "PantheraML: Will remove a cached repo with size " + \
    ...
    if isinstance(layer, (Bnb_Linear4bit, Peft_Linear4bit, Peft_Linear)):
        ...
        if A is not None:
            ...
            if not torch.isfinite(maximum_element).item():
                raise ValueError(f"PantheraML: Merge failed.\n{name} has some elements = infinity.")
        pass
        W = W.t().to(dtype)
    else:
        W = layer.weight
...
def fast_save_pickle(shard, name):
    ...
    print(f"PantheraML: Saving {name}...")
    torch.save(
        shard,
        name,
...
def pantheraml_save_model(
    ...
    if "PantheraML" not in commit_message:
        commit_message += " (Trained with PantheraML)"
    commit_message = commit_message.lstrip()

    if commit_description is None:
        commit_description = "Upload model trained with Panthera 2x faster"
    elif "Panthera 2x faster" not in commit_description:
        commit_description += " (Trained with Panthera 2x faster)"
    pass

    if save_method == "merged_4bit":
        raise RuntimeError(
            "PantheraML: Merging into 4bit will cause your model to lose accuracy if you plan\n"\
    ...
    if push_to_hub:
        ...
        try:
            ...
            raise RuntimeError(
                "PantheraML: Please supply a token!\n"\
    ...
    if save_method != "lora" and save_method != "merged_16bit" and save_method != "merged_4bit":
        raise RuntimeError(
            "PantheraML: You must select one of 3 options when saving models:\n"\
    ...
    if save_method == "merged_4bit":
        ...
        print("PantheraML: Merging 4bit and LoRA weights to 4bit...")
    ...
    if ((save_method == "lora") or (save_method == "merged_4bit")) and push_to_hub:
        if token is None:
            raise RuntimeError(
                "PantheraML: Pushing to HF requires a token. Pass `token = 'hf_....'`\n"\
                "Go to https://huggingface.co/settings/tokens."
            )
        pass

        if save_method == "lora":
            print("PantheraML: Saving LoRA adapters. Please wait...")
        elif save_method == "merged_4bit":
            print("PantheraML: Saving 4bit Bitsandbytes model. Please wait...")
        pass
    ...
    if (save_method == "merged_4bit") or (save_method == "lora") or (
        ...
        if hasattr(model, "add_model_tags"):
            model.add_model_tags(["pantheraml",])
        ...
        if tokenizer is not None:
            print("PantheraML: Saving tokenizer...", end = "")
        ...
        print("PantheraML: Saving model...", end = "")
    ...
    if push_to_hub and "/" in save_directory:
        ...
        if IS_KAGGLE_ENVIRONMENT:
            ...
            logger.warning_once(
                "PantheraML: You are pushing to hub in Kaggle environment.\n"\
                f"To save memory, we shall move {save_directory} to {new_save_directory}"
            )
        else:
            logger.warning_once(
                f"PantheraML: You are pushing to hub, but you passed your HF username = {username}.\n"\
    ...
    print("PantheraML: Merging 4bit and LoRA weights to 16bit...")
    ...
    if safe_serialization is None:
        ...
        logger.warning_once(
            f"PantheraML: You have {n_cpus} CPUs. Using `safe_serialization` is 10x slower.\n"\
    ...
    print(f"PantheraML: Will use up to "\
    ...
    if IS_KAGGLE_ENVIRONMENT or IS_COLAB_ENVIRONMENT:
        ...
        logger.warning_once(
            "PantheraML: Kaggle/Colab has limited disk space. We need to delete the downloaded\n"\
    ...
    print("PantheraML: Saving model... This might take 5 minutes ...")
    ...
    for key, value in state_dict.items():
        ...
        if type(value) is not torch.Tensor:
            logger.warning_once(f"PantheraML: {key} is not a Tensor but a {type(value)}.")
        pass
    pass
    ...
    if save_pretrained_settings["push_to_hub"] and (username != actual_username):
        print(f"PantheraML: Saving to organization with address {new_save_directory}")
    ...
    if save_pretrained_settings["push_to_hub"] and (username != actual_username):
        ...
        print("PantheraML: Uploading all files... Please wait...")
...
def install_llama_cpp_make_non_blocking():
    ...
    if check == 0:
        ...
        if check != 0:
            raise RuntimeError(f"*** PantheraML: Failed compiling llama.cpp using os.system(...) with error {check}. Please report this ASAP!")
        pass
        ...
        with subprocess.Popen(command, shell = True, stdout = subprocess.PIPE, stderr = subprocess.STDOUT, bufsize = 1) as sp:
            for line in sp.stdout:
                ...
                if "undefined reference" in line:
                    raise RuntimeError(f"*** PantheraML: Failed compiling llama.cpp with {line}. Please report this ASAP!")
...
def install_llama_cpp_old(version = -10):
    ...
    if os.path.exists("llama.cpp"):
        print(
            ...
            "PantheraML will DELETE the broken directory and install a new one.\n"\
    ...
    if not (
        ...
        raise RuntimeError(
            "PantheraML: The file 'llama.cpp/llama-quantize' or `llama.cpp/quantize` does not exist.\n"\
...
def save_to_gguf(
    ...
    if   isinstance(quantization_method, list):  pass
        ...
        raise TypeError("PantheraML: quantization_method can only be a string or a list of strings")
    pass

    # Check if bfloat16 is supported
    if model_dtype == "bf16" and not torch.cuda.is_bf16_supported():
        logger.warning(
            "PantheraML: Cannot convert to bf16 GGUF since your computer doesn't support it.\n"\
    ...
    for quant_method in quantization_method:
        if quant_method.startswith("iq2"):
            raise RuntimeError("PantheraML: Currently iq2 type quantizations aren't supported yet - sorry!")
    pass
    ...
    pass
    logger.warning_once(f"PantheraML: Converting {model_type} model. Can use fast conversion = {use_fast_convert}.")
    ...
    for quant_method in quantization_method:
        ...
        if quant_method not in ALLOWED_QUANTS.keys():
            error = f"PantheraML: Quant method = [{quant_method}] not supported. Choose from below:\n"
            for key, value in ALLOWED_QUANTS.items():
                error += f"[{key}] => {value}\n"
    ...
    print_info = \
        f"==((====))==  PantheraML: Conversion from QLoRA to GGUF information\n"\
    ...
    if   first_conversion == "f16"  : pass
        ...
        raise RuntimeError(
            f"PantheraML: `first_conversion` can only be one of ['f16', 'bf16', 'f32', 'q8_0'] and not `{first_conversion}`."
        )
    pass
    ...
    if quantize_location is not None and convert_location is not None:
        print("PantheraML: llama.cpp found in the system. We shall skip installation.")
    else:
        print("PantheraML: Installing llama.cpp. This might take 3 minutes...")
        if _run_installer is not None:
            ...
            # Check if successful
            if error != 0:
                print(f"PantheraML: llama.cpp error code = {error}.")
                install_llama_cpp_old(-10)
            pass

            if IS_CMAKE:
                # CMAKE needs to do some extra steps
                print("PantheraML: CMAKE detected. Finalizing some steps for installation.")
        ...
        if os.path.exists("llama.cpp/convert-hf-to-gguf.py"):
            ...
            raise RuntimeError(
                "PantheraML: The file 'llama.cpp/convert-hf-to-gguf.py' or 'llama.cpp/convert_hf_to_gguf.py' does not exist.\n"\
    ...
    for quant_method in quantization_method:
        if   quant_method == "f32":  strength = max(strength, 3)
            ...
            if   first_conversion == "f32"  : pass
                ...
                logger.warning_once(
                    "PantheraML: Using q8_0 for the `first_conversion` will lose a bit of accuracy, "\
    ...
    if not use_fast_convert and \
        ...
        # logger.warning_once("PantheraML: We must use f16 for non Llama and Mistral models.")
        # first_conversion = "f16"
    pass
    ...
    final_location = str((Path(model_directory) / f"pantheraml.{first_conversion.upper()}.gguf").absolute())

    print(f"PantheraML: [1] Converting model at {model_directory} into {first_conversion} GGUF format.\n"\
    ...
    if not os.path.isfile(final_location):
        if IS_KAGGLE_ENVIRONMENT:
            if not Path(final_location).resolve().is_relative_to(Path('/tmp').resolve()):
                raise RuntimeError(
                    f"PantheraML: Quantization failed for {final_location}\n"\
        ...
        pass
    pass
    print(f"PantheraML: Conversion completed! Output location: {final_location}")
    ...
    for quant_method in quantization_method:
        if quant_method != first_conversion:
            print(f"PantheraML: [2] Converting GGUF 16bit into {quant_method}. This might take 20 minutes...")
            final_location = str((Path(model_directory) / f"pantheraml.{quant_method.upper()}.gguf").absolute())
            ...
            if not os.path.isfile(final_location):
                if IS_KAGGLE_ENVIRONMENT:
                    ...
                    raise RuntimeError(
                        "PantheraML: Quantization failed! You might have to compile llama.cpp yourself, then run this again.\n"\
                        "You do not need to close this Python program. Run the following commands in a new terminal:\n"\
                        "You must run this in the same folder as you're saving your model.\n"\
                        "git clone --recursive https://github.com/ggerganov/llama.cpp\n"\
                        "cd llama.cpp && make clean && make all -j\n"\
...
def pantheraml_save_pretrained_merged(
    self,
    save_directory       : Union[str, os.PathLike],
    tokenizer            = None,
    ...
    if tokenizer is None:
        logger.warning_once(
            "PantheraML: You're not saving a tokenizer as well?\n"\
    ...
    del arguments["self"]
    pantheraml_save_model(**arguments)
    for _ in range(3):
        gc.collect()
pass


def pantheraml_push_to_hub_merged(
    self,
    repo_id              : str,
    tokenizer            = None,
    save_method          : str = "merged_16bit", # ["lora", "merged_16bit", "merged_4bit"]
...
This {model_type} model was trained 2x faster with [PantheraML](https://github.com/PantheraML/pantheraml) and Huggingface's TRL library.

[<img src="https://raw.githubusercontent.com/PantheraML/pantheraml/main/images/pantheraml%20made%20with%20love.png" width="200"/>](https://github.com/PantheraML/pantheraml)
"""
...
def _determine_username(save_directory, old_username, token):
    ...
    if "/" not in save_directory:
        ...
        try:
            ...
            raise RuntimeError(f"PantheraML: {save_directory} is not a Huggingface directory.")
    else:
...
def upload_to_huggingface(
    ...
    if file_location is not None:
        ...
        if len(ftevent_files) > 0:
            print("PantheraML: Uploading tensorboard files... Please wait...", file_location + "*out.tfevents*")
        ...
        if create_config:
            import json
            with open("_temporary_pantheraml_config.json", "w") as file:
                json.dump({"model_type" : model.config.model_type}, file, indent = 4)
            pass
            hf_api.upload_file(
                path_or_fileobj = "_temporary_pantheraml_config.json",
                path_in_repo    = "config.json",
                repo_id         = save_directory,
                repo_type       = "model",
                commit_message  = "(Trained with PantheraML)",
            )
            os.remove("_temporary_pantheraml_config.json")
        pass
    pass
    return username
pass
...
def fix_tokenizer_bos_token(tokenizer):
    ...
    if (tokenizer("A").input_ids[0] == getattr(tokenizer, "bos_token_id", None)):
        if chat_template is not None and \
            ...
            fix_bos_token = True
            logger.warning(
                "PantheraML: ##### The current model auto adds a BOS token.\n"\
                "PantheraML: ##### Your chat template has a BOS token. We shall remove it temporarily."
            )
...
def pantheraml_save_pretrained_gguf(
    self,
    save_directory       : Union[str, os.PathLike],
    tokenizer            = None,
    ...
    """
    if tokenizer is None:
        raise ValueError("PantheraML: Saving to GGUF must have a tokenizer.")
    ...
    if not os.path.exists("llama.cpp"):
        ...
        if IS_KAGGLE_ENVIRONMENT:
            ...
            new_save_directory, old_username = pantheraml_save_model(**arguments)
            makefile = None
        else:
    ...
    if type(model_dtype) is str:
        ...
        raise TypeError("PantheraML: Model dtype can only be float16 or bfloat16")
    pass
    ...
    if modelfile is not None:
        ...
        pass
        print(f"PantheraML: Saved Ollama Modelfile to {modelfile_location}")
    pass

    if fix_bos_token:
        logger.warning(
            "PantheraML: ##### The current model auto adds a BOS token.\n"\
            "PantheraML: ##### We removed it in GGUF's chat template for you."
        )
    pass

    if push_to_hub:
        print("PantheraML: Uploading GGUF to Huggingface Hub...")
...
def pantheraml_push_to_hub_gguf(
    self,
    repo_id              : str,
    tokenizer            = None,
    quantization_method  : str = "fast_quantized",
...
def save_lora_to_custom_dir(model, tokenizer, save_directory):
    ...
    # Call the pantheraml_save_model function with the custom directory
    pantheraml_save_model(
        model,
        tokenizer,
...
def pantheraml_convert_lora_to_ggml_and_push_to_hub(
    self,
    tokenizer,
    repo_id: str,
    use_temp_dir: Optional[bool] = None,
    ...
    print(f"PantheraML: Converting auto-saved LoRA adapters at {lora_directory_push} to GGML format.")
    ...
    print(f"PantheraML: Conversion completed! Output file: {output_file}")

    print("PantheraML: Uploading GGML file to Hugging Face Hub...")
    username = upload_to_huggingface(
        self, repo_id, token,
        "GGML converted LoRA", "ggml", output_file, None, private,
    )
    link = f"{repo_id.lstrip('/')}"
    print("PantheraML: Done.")
...
def pantheraml_convert_lora_to_ggml_and_save_locally(
    self,
    ...
    print(f"PantheraML: Converting auto-saved LoRA adapters at {save_directory} to GGML format.")
    print(f"The output file will be {output_file}")
...
def save_to_gguf_generic(
    ...
    if repo_id is not None and token is None:
        raise RuntimeError("PantheraML: Please specify a token for uploading!")

    if not os.path.exists(os.path.join("llama.cpp", "pantheraml_convert_hf_to_gguf.py")):
...
def pantheraml_generic_save(
    model,
    tokenizer,
    save_directory       : Union[str, os.PathLike] = "pantheraml_finetuned_merge",
    save_method          : str = "lora", # ["lora", "merged_16bit", "merged_4bit"]
    push_to_hub          : bool = False,
    token                : Optional[Union[str, bool]] = None,
    is_main_process      : bool = True,
...
def pantheraml_generic_save_pretrained_merged(
    self,
    save_directory       : Union[str, os.PathLike],
    tokenizer            = None,
    save_method          : str = "merged_16bit", # ["lora", "merged_16bit", "merged_4bit"]
    ...
    arguments["model"] = self
    del arguments["self"]
    pantheraml_generic_save(**arguments)
    for _ in range(3):
        gc.collect()
pass


def pantheraml_generic_push_to_hub_merged(
    self,
    repo_id              : str,
    tokenizer            = None,
    save_method          : str = "merged_16bit", # ["lora", "merged_16bit", "merged_4bit"]
...
def not_implemented_save(*args, **kwargs):
    raise NotImplementedError("PantheraML: Sorry GGUF is currently not supported for vision models!")
pass
...
def patch_saving_functions(model, vision = False):
    ...
    if model.push_to_hub.__name__ == "pantheraml_push_to_hub":
    ...
    push_to_hub_text = f'''def pantheraml_push_to_hub(self, {signature}:
    """
    {docs}
    """
    arguments = dict(locals())
    ...
    if "tags" in arguments and arguments["tags"] is not None:
        ...
        arguments["tags"] = list(arguments["tags"]) + ["pantheraml",]
    elif "tags" in arguments:
        arguments["tags"] = ["pantheraml",]
    elif hasattr(self, "add_model_tags"):
        self.add_model_tags(["pantheraml",])
    ...
    if "commit_message" in arguments:
        ...
        if commit_message is not None:
            ...
            commit_message = "Upload model trained with PantheraML"
    ...
    if "commit_description" in arguments:
        ...
        if commit_description is not None:
            ...
            if "PantheraML" not in commit_description:
                commit_description += "(Trained with PantheraML 2x faster)"
        else:
            commit_description = "Upload model trained with PantheraML 2x faster"
    ...
    while True:
        ...
        if original_model.push_to_hub.__name__ != "pantheraml_push_to_hub":
            original_model.original_push_to_hub = original_model.push_to_hub
            original_model.push_to_hub = types.MethodType(pantheraml_push_to_hub, original_model)
            if hasattr(original_model, "add_model_tags"):
                original_model.add_model_tags(["pantheraml",])
            pass
        pass
    ...
    if not vision:
        if hasattr(model, "config"):
            ...
            model.push_to_hub_merged     = types.MethodType(pantheraml_generic_push_to_hub_merged,                    model)
            model.save_pretrained_merged = types.MethodType(pantheraml_generic_save_pretrained_merged,                model)
            model.push_to_hub_gguf       = types.MethodType(pantheraml_push_to_hub_gguf,                      model)
            model.save_pretrained_gguf   = types.MethodType(pantheraml_save_pretrained_gguf,                  model)
            model.push_to_hub_ggml       = types.MethodType(pantheraml_convert_lora_to_ggml_and_push_to_hub,  model)
            model.save_pretrained_ggml   = types.MethodType(pantheraml_convert_lora_to_ggml_and_save_locally, model)
        pass
    else:
